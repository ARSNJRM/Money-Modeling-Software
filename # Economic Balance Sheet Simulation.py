...
class EntryType(Enum):
    ...
    BOND = "bond"
    SHARE = "share"   # Add a new type: share
...
class Agent:
    ...
    def get_net_worth(self) -> float:
      # Revise to adapt to share which is not counted in net worth
        total_assets = sum(entry.amount for entry in self.assets if entry.type != EntryType.SHARE)
        total_liabilities = sum(entry.amount for entry in self.liabilities if entry.type != EntryType.SHARE)
...
class AssetLiabilityPair:
    def __init__(self,
        ...
        if self.type == EntryType.SHARE.value:
            # Find the value for share
            if self.type == EntryType.SHARE.value:
                if not self.liability_holder:
                    raise ValueError("Share issuer not specified")

            issuer_net_worth = self.liability_holder.get_net_worth()
            if issuer_net_worth <= 0:
                raise ValueError("Issuer must have positive net worth")

            self.value_per_share = issuer_net_worth / self.amount
    ...
    def create_entries(self) -> Tuple[BalanceSheetEntry, Optional[BalanceSheetEntry]]:
        ...
        # For share pairs
            asset_entry = BalanceSheetEntry(
                ...
                maturity_date=None,
                settlement_details=SettlementDetails(SettlementType.NONE, "shares"),
                name=None,
                issuance_time='t0'
            )
    ...
    def calculate_share_value(self): #To give value per share
        if self.type != EntryType.SHARE.value:
            return

        if not self.asset_holder or not self.liability_holder:
            raise ValueError("Missing asset/liability holder for shares")
    ...
    def display_balance_sheets(self, time_point: str):
        ...
        for agent in current_agents:
            ...
            for asset in agent.assets:
                ...
                elif asset.type == EntryType.SHARE:  # 新增股权资产显示
                    entry_type = f"shares held: {asset.amount} {asset.denomination} (issued by {asset.counterparty})"
                else:
            ...
            for liability in agent.liabilities:
                ...
                if liability.type == EntryType.SHARE:
                    # To realize the special format shown in balance sheet
                    print(f" - ({liability.type.value}): {liability.amount} {liability.denomination}")
...
def create_asset_liability_pair_interactive(system: EconomicSystem, default_denomination: Optional[str] = None):
    ...
    try:
        ...
        if entry_type_choice == 1:
            ...
            elif type_idx == 5:  # Share creation logic
                entry_type = EntryType.SHARE
                issuer_agent = liability_holder

                # Validate the issuer's net worth
                if issuer_agent.get_net_worth() <= 0:
                    print(f"Error: Issuer {issuer_agent.name} h
as insufficient net worth ({issuer_agent.get_net_worth()})")
                    return
                # Get the number of shares to issue
                try:
                    number_of_shares = float(input("Enter total shares to issue: "))
                    if number_of_shares <= 0:
                        print("Error: Must issue at least 1 share")
                        return
                except ValueError:
                    print("Error: Invalid number format")
                    return

                # Check if the holder already holds shares from the issuer
                existing_shares = [
                    a for a in asset_holder.assets
                    if a.type == EntryType.SHARE
                    and a.counterparty == issuer_agent.name
                ]
                if existing_shares:
                    print(f"Error: {asset_holder.name} already holds shares from {issuer_agent.
                    return

                # Create the share pair using a unified parameter structure
                pair = AssetLiabilityPair(
                    time=datetime.now(),
                    ...
                    asset_holder=asset_holder,
                    liability_holder=issuer_agent,  # Use the selected liability holder
                    asset_name=None
                )
                try:
                    pair.calculate_share_value()
                    asset_entry, liability_entry = pair.create_entries()

                    # Check for duplicate entries before adding them
                    if any(e.matches(asset_entry) for e in asset_holder.assets):
                        print("Error: Duplicate asset entry")
                        return
                    if any(e.matches(liability_entry) for e in issuer_agent.liabilities):
                        print("Error: Duplicate liability entry")
                        return

                    system.create_asset_liability_pair(pair)

                    print("\nShare created successfully!")
                    print(f"- Holder: {asset_holder.name}")
                    print(f"- Issuer: {issuer_agent.name}")
                    print(f"- Value per share: {pair.value_per_share:.2f}")

                except ValueError as e:
                    print(f"Error: {str(e)}")
                    # Rollback if partial creation occurred
                    asset_holder.remove_asset(asset_entry)
                    issuer_agent.remove_liability(liability_entry)
                return
            else:
