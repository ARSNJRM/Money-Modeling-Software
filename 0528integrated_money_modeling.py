...
class EconomicSystem:
    ...
    def deposit(self, user_agent: 'Agent', bank_agent: 'Agent', amount: float, denomination: str = "USD"):
        """
        User deposit: increase user's asset (deposit) and bank's liability (deposit liability).
        
        :param user_agent: Agent who deposits cash (asset holder).
        :param bank_agent: Bank agent holding the liability for the deposit.
        :param amount: Amount of cash to deposit; must be positive.
        :param denomination: Currency denomination, default is "USD".
        ...
        pair = AssetLiabilityPair(
            ...
            asset_holder=user_agent,
            liability_holder=bank_agent
        )
        self.create_asset_liability_pair(pair)
        print(f"{user_agent.name} deposited {amount} {denomination} successfully.")


    def withdraw(self, user_agent: 'Agent', bank_agent: 'Agent', amount: float, denomination: str = "USD"):
        """
        User withdrawal: check if user's deposit balance is sufficient,
        then reduce user's deposit asset and bank's deposit liability accordingly.
        
        :param user_agent: Agent who withdraws cash (asset holder).
        :param bank_agent: Bank agent holding the deposit liability.
        :param amount: Amount of cash to withdraw; must be positive.
        :param denomination: Currency denomination, default is "USD".
        :raises ValueError: if amount is not positive or if balance is insufficient.
        """
        if amount <= 0:
            raise ValueError("Withdrawal amount must be greater than 0")

        # Calculate total deposit balance of the user in the specified denomination
        deposits = [asset for asset in user_agent.assets if asset.type == EntryType.DEPOSIT and asset.denomination == denomination]
        total_deposit = sum(d.current_book_value for d in deposits)

        if total_deposit < amount:
            raise ValueError(f"Insufficient balance, current balance is {total_deposit} {denomination}")

        # Deduct from deposit asset-liability pairs one by one until withdrawal amount is fulfilled
        remaining = amount
        for deposit_asset in deposits:
            if remaining <= 0:
                break
            deduct_amount = min(deposit_asset.current_book_value, remaining)

            # Create a new asset-liability pair to represent the deduction (withdrawal)
            pair = AssetLiabilityPair(
                time=self.current_time_state,
                type=EntryType.DEPOSIT.value,
            ...
            # Remove the original deposit asset and corresponding bank liability (simplified approach)
            user_agent.remove_asset(deposit_asset)
            bank_agent.remove_liability(next(
                (l for l in bank_agent.liabilities
                if l.counterparty == user_agent.name and l.type == EntryType.DEPOSIT and l.current_book_value == deposit_asset.current_book_value),
                None))

            # If original deposit asset value is greater than deducted amount,
            # create a new asset-liability pair for the remaining balance
            if deposit_asset.current_book_value > deduct_amount:
                remainder_pair = AssetLiabilityPair(
                    time=self.current_time_state,
                    type=EntryType.DEPOSIT.value,
                    amount=deposit_asset.current_book_value - deduct_amount,
                    denomination=denomination,
                    maturity_type=MaturityType.ON_DEMAND,
                    maturity_date=None,
                    settlement_type=SettlementType.NONE,
                ...
                self.create_asset_liability_pair(remainder_pair)

            remaining -= deduct_amount

            # Note: No new asset-liability pair is created for the deducted amount,
            # as the cash is considered withdrawn from the system.

        print(f"{user_agent.name} withdrew {amount} {denomination} successfully.")
