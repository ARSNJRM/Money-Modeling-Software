...
class EntryType(Enum):
    ...
    INTRADAY_IOU = "interbank_intraday_iou"
    OVERNIGHT_LOAN = "interbank_overnight_loan"
...
class EconomicSystem:
    ...
    def create_scheduled_asset_liability_pair(self, **params):
        """Create an asset-liability pair from scheduled action parameters"""
        # Get agent objects from names
        asset_holder = self.agents[params['asset_holder']]
        liability_holder = self.agents[params['liability_holder']] if params['liability_holder'] else None
        
        # Create the AssetLiabilityPair object
        pair = AssetLiabilityPair(
            time=self.current_time_state,
            type=params['type'],
            amount=params['amount'],
            denomination=params['denomination'],
            maturity_type=params['maturity_type'],
            maturity_date=params['maturity_date'],
            settlement_type=params['settlement_type'],
            settlement_denomination=params['settlement_denomination'],
            asset_holder=asset_holder,
            liability_holder=liability_holder,
            cash_flow_at_maturity=params['cash_flow_at_maturity'],
            asset_name=params['asset_name'],
            bond_type=params['bond_type'],
            coupon_rate=params['coupon_rate']
        )
        
        # Create the pair using the existing method
    ...
    def run_user_scheduled_actions(self, time_point: int):
        self.current_time_state = time_point
        ...
        action_map = {
            'Create Asset–Liability Pair': self.create_scheduled_asset_liability_pair,
...
def select_agent_with_constraints(system: EconomicSystem, prompt: str, entry_type: EntryType, as_asset_holder: bool, exclude_agent: Optional[Agent] = None) -> Optional[Agent]:
    """Select an agent with constraint validation and optional exclusion"""
    all_agents = list(system.agents.items())
    if not all_agents:
        print("No agents available.")
        return None
    
    # Filter agents based on constraints and exclusions
    valid_agents = []
    for name, agent in all_agents:
        # Skip excluded agent (to prevent same agent being asset and liability holder)
        if exclude_agent and agent.name == exclude_agent.name:
            continue
            
        if as_asset_holder:
            is_valid, _ = AssetLiabilityConstraints.validate_asset_holder(entry_type, agent.type)
        else:
            is_valid, _ = AssetLiabilityConstraints.validate_liability_holder(entry_type, agent.type)
        
        if is_valid:
            valid_agents.append((name, agent))
    
    if not valid_agents:
        if exclude_agent:
            print(f"No valid agents available (excluding {exclude_agent.name} to prevent self-transactions).")
        else:
            if as_asset_holder:
                _, error_msg = AssetLiabilityConstraints.validate_asset_holder(entry_type, list(system.agents.values())[0].type)
            else:
                _, error_msg = AssetLiabilityConstraints.validate_liability_holder(entry_type, list(system.agents.values())[0].type)
            print(f"No valid agents available. {error_msg}")
        return None
    
    print("Available agents:")
    for i, (name, agent) in enumerate(valid_agents):
        print(f"  {i + 1}. {name} ({agent.type.value})")
    
    while True:
        choice = get_input(f"{prompt} (Enter number): ", int)
        if 1 <= choice <= len(valid_agents):
            return valid_agents[choice - 1][1]
        else:
            print("Invalid number")
...
def select_entry_type_with_submenus():
    """Custom entry type selection with hierarchical submenus"""
    # Create a list of main entry types excluding grouped types
    main_entry_types = [
    ...
    print("  1. non-financial entry")
    print("  2. payable/deliverable")
    print("  3. security entry")
    print("  4. bank specific entry")
    print("  5. default entry")
    
    while True:
        choice = get_input("Select entry type (Enter number): ", int)
        if choice == 1:
            return EntryType.NON_FINANCIAL, None
        elif choice == 2:
            # Show payable/deliverable submenu
            return select_payable_deliverable_type()
        elif choice == 3:
            # Show security submenu
            return select_security_type()
        elif choice == 4:
            # Show bank specific entries submenu
            return select_bank_specific_type()
        elif choice == 5:
            return EntryType.DEFAULT, None
        else:
            print("Invalid number")

def select_bank_specific_type():
    """Submenu for selecting bank specific entry types"""
    print("\nBank specific entry types:")
    print("  1. bank client loan")
    print("  2. bank deposit") 
    print("  3. reserve deposit")
    print("  4. interbank claims")
    
    while True:
        choice = get_input("Select bank specific entry type (Enter number): ", int)
        if choice == 1:
            return EntryType.LOAN, None
        elif choice == 2:
            return EntryType.DEPOSIT, None
        elif choice == 3:
            return EntryType.RESERVE_DEPOSIT, None
        elif choice == 4:
            # Show interbank claims submenu
            return select_interbank_claims_type()
        else:
            print("Invalid number")

def select_security_type():
    """Submenu for selecting security types"""
    print("\nSecurity types:")
    print("  1. bond")
    print("  2. share")
    
    while True:
        choice = get_input("Select security type (Enter number): ", int)
        if choice == 1:
            # Show bond submenu
            return select_bond_type()
        elif choice == 2:
            return EntryType.SHARE, None
        else:
            print("Invalid number")

def select_interbank_claims_type():
    """Submenu for selecting interbank claims types"""
    interbank_types = [
        EntryType.INTRADAY_IOU,
        EntryType.OVERNIGHT_LOAN
    ]
    
    print("\nInterbank claims types:")
    for i, entry_type in enumerate(interbank_types):
        print(f"  {i + 1}. {entry_type.value}")
    
    while True:
        choice = get_input("Select interbank claims type (Enter number): ", int)
        if 1 <= choice <= len(interbank_types):
            return interbank_types[choice - 1], None
        else:
            print("Invalid number")

def select_payable_deliverable_type():
    """Submenu for selecting payable/deliverable types"""
    payable_types = [
        EntryType.PAYABLE,
        EntryType.DELIVERY_CLAIM
    ]
    
    print("\nPayable/Deliverable types:")
    for i, entry_type in enumerate(payable_types):
        print(f"  {i + 1}. {entry_type.value}")
    
    while True:
        choice = get_input("Select payable/deliverable type (Enter number): ", int)
        if 1 <= choice <= len(payable_types):
            return payable_types[choice - 1], None
        else:
            print("Invalid number")

def select_bond_type():
    """Submenu for selecting bond categories and types"""
    print("\nBond categories:")
    print("  1. Corporate/Private Bond")
    print("  2. Government Bond") 
    print("  3. Central Bank Bond")
    
    while True:
        category_choice = get_input("Select bond category (Enter number): ", int)
        if category_choice == 1:
            # Corporate/Private bonds
            bond_types = [
                (EntryType.BOND_ZERO_COUPON, BondType.ZERO_COUPON),
                (EntryType.BOND_COUPON, BondType.COUPON),
                (EntryType.BOND_AMORTIZING, BondType.AMORTIZING)
            ]
            type_prefix = "Corporate/Private"
        elif category_choice == 2:
            # Government bonds  
            bond_types = [
                (EntryType.GOVERNMENT_BOND_ZERO_COUPON, BondType.ZERO_COUPON),
                (EntryType.GOVERNMENT_BOND_COUPON, BondType.COUPON),
                (EntryType.GOVERNMENT_BOND_AMORTIZING, BondType.AMORTIZING)
            ]
            type_prefix = "Government"
        elif category_choice == 3:
            # Central bank bonds
            bond_types = [
                (EntryType.CENTRAL_BANK_BOND_ZERO_COUPON, BondType.ZERO_COUPON),
                (EntryType.CENTRAL_BANK_BOND_COUPON, BondType.COUPON),
                (EntryType.CENTRAL_BANK_BOND_AMORTIZING, BondType.AMORTIZING)
            ]
            type_prefix = "Central Bank"
        else:
            print("Invalid category number")
            continue
        
        print(f"\n{type_prefix} bond types:")
        for i, (entry_type, bond_type) in enumerate(bond_types):
            print(f"  {i + 1}. {bond_type.name.replace('_', ' ').title()}")
        
        while True:
            type_choice = get_input("Select bond type (Enter number): ", int)
            if 1 <= type_choice <= len(bond_types):
                entry_type, bond_type = bond_types[type_choice - 1]
                return entry_type, bond_type
            else:
                print("Invalid number")
    ...
    while True:
        ...
        try:
            if choice == 1:
                ...
                # First, let user select entry type to know constraints
                entry_type, bond_type = select_entry_type_with_submenus()
                
                # Then select asset holder with constraints
                asset_holder = select_agent_with_constraints(system, "select asset holder", entry_type, as_asset_holder=True)
                if not asset_holder:
                    continue
                ...
                if entry_type == EntryType.NON_FINANCIAL:
                    ...
                    liability_holder = select_agent_with_constraints(system, "select liability holder", entry_type, as_asset_holder=False, exclude_agent=asset_holder)
                    ...
                    liability_holder = select_agent_with_constraints(system, "select liability holder (central bank)", entry_type, as_asset_holder=False, exclude_agent=asset_holder)
                    if not liability_holder:
                        continue

                    amount = get_input("Enter reserve deposit amount: ", float)
                    denomination = get_input("Enter denomination (typically reserves or central bank currency): ")
                    maturity_type = MaturityType.ON_DEMAND
                    maturity_date = None
                    if maturity_type == MaturityType.FIXED_DATE:
                        maturity_date = get_input("Enter maturity date: ", int, choices=[1, 2])
                    ...
                    system.create_asset_liability_pair(pair)
                    print("Reserve deposit created successfully.")

                # Handle bond types
                elif entry_type in [EntryType.BOND_ZERO_COUPON, EntryType.BOND_COUPON, EntryType.BOND_AMORTIZING,
                    ...
                    liability_holder = select_agent_with_constraints(system, "select liability holder (bond issuer)", entry_type, as_asset_holder=False, exclude_agent=asset_holder)
                    if not liability_holder:
                        continue

                    amount = get_input("Enter bond face value: ", float)
                    denomination = get_input("Enter denomination (USD, EUR, etc.): ")
                    maturity_type = MaturityType.FIXED_DATE
                    maturity_date = get_input("Enter maturity date: ", int, choices=[1, 2])
                    ...
                    cash_flow_at_maturity = amount  # Default for zero coupon
                    
                    # Handle coupon rate for coupon and amortizing bonds
                    if entry_type in [EntryType.BOND_COUPON, EntryType.BOND_AMORTIZING,
                        ...
                        coupon_rate = get_input("Enter coupon/interest rate (e.g., 0.05 for 5%): ", float)
                        if entry_type in [EntryType.BOND_COUPON, EntryType.GOVERNMENT_BOND_COUPON, EntryType.CENTRAL_BANK_BOND_COUPON]:
                            # For coupon bonds, cash flow at maturity includes final coupon + principal
                            cash_flow_at_maturity = amount * (1 + coupon_rate)
                        elif entry_type in [EntryType.BOND_AMORTIZING, EntryType.GOVERNMENT_BOND_AMORTIZING, EntryType.CENTRAL_BANK_BOND_AMORTIZING]:
                            # For amortizing bonds, calculate total expected cash flow
                            if maturity_date == 1:
                                cash_flow_at_maturity = amount * (1 + coupon_rate)
                            else:
                                # Two payments for maturity at t=2
                                cash_flow_at_maturity = amount * (1 + coupon_rate + 0.5 * coupon_rate)
                    ...
                    system.create_asset_liability_pair(pair)
                    print(f"{bond_type.name.replace('_', ' ').title()} bond created successfully.")
                
                else:
                    # Check if this entry type has liability constraints
                    if entry_type in [EntryType.SHARE, EntryType.INTRADAY_IOU, EntryType.OVERNIGHT_LOAN, EntryType.LOAN]:
                        liability_holder = select_agent_with_constraints(system, "select liability holder", entry_type, as_asset_holder=False, exclude_agent=asset_holder)
                    else:
                ...
                action_types = [
                    'Create Asset–Liability Pair', 'Repay Loan', 'Repay Bond', 
                ...
                params = {}
                if action_type == 'Create Asset–Liability Pair':
                    # First, let user select entry type to know constraints
                    entry_type, bond_type = select_entry_type_with_submenus()
                    ...
                    if entry_type == EntryType.NON_FINANCIAL:
                        ...
                        params = {
                            ...
                            'settlement_type': settlement_type,
                            'settlement_denomination': settlement_denomination,
                            'asset_holder': asset_holder.name,
                            'liability_holder': liability_holder.name if liability_holder else None,
                            'cash_flow_at_maturity': cash_flow_at_maturity,
                            'asset_name': asset_name,
                            'bond_type': bond_type,
                            'coupon_rate': coupon_rate
                        ...
                        liability_holder = select_agent_with_constraints(system, "select liability holder", entry_type, as_asset_holder=False)
                        if not liability_holder:
                            continue
                        ...
                        params = {
                            ...
                            'settlement_denomination': settlement_denomination,
                            'asset_holder': asset_holder.name,
                            'liability_holder': liability_holder.name,
                            'cash_flow_at_maturity': cash_flow_at_maturity,
                            'asset_name': asset_name,
                            'bond_type': bond_type,
                            'coupon_rate': coupon_rate
                        ...
                        liability_holder = select_agent_with_constraints(system, "select liability holder (central bank)", entry_type, as_asset_holder=False)
                        if not liability_holder:
                            continue
                    ...
                    # Handle bond types in Actions menu too
                    elif entry_type in [EntryType.BOND_ZERO_COUPON, EntryType.BOND_COUPON, EntryType.BOND_AMORTIZING,
                                       EntryType.GOVERNMENT_BOND_ZERO_COUPON, EntryType.GOVERNMENT_BOND_COUPON, EntryType.GOVERNMENT_BOND_AMORTIZING,
                                       EntryType.CENTRAL_BANK_BOND_ZERO_COUPON, EntryType.CENTRAL_BANK_BOND_COUPON, EntryType.CENTRAL_BANK_BOND_AMORTIZING]:
                        liability_holder = select_agent_with_constraints(system, "select liability holder (bond issuer)", entry_type, as_asset_holder=False, exclude_agent=asset_holder)
                ...
                if params or action_type == 'Process Intraday Settlements': 
                    ...
                    print(f"Operation '{action_type}' scheduled for timepoint {time_point}.")
                else:
                     print("Operation cancelled")
            

            elif choice == 5:
                for time_point in range(1, 3):
                    print(f"--- Operating {time_point} ---")
                    system.run_user_scheduled_actions(time_point)
